...
class RequestService {
    ...
    public func requestTask(patch: URL,
        ...
        let sessionRequest = SessionManager.shared.request(request)
            .customValidate()
    ...
    public func uploadRequestTask(patch: URL,
        ...
        var request: URLRequest = URLRequest(url: patch)
            ...
            request.httpBody = body
            request.allHTTPHeaderFields = headerParametrs
        let sessionRequest = SessionManager.shared.upload(body!, with: request)
            .customValidate()
            .response { requestResponse in
                response(requestResponse.data, requestResponse.response, requestResponse.error)
        }
    ...
    public func downloadFileRequestTask(patch: URL,
        ...
        let sessionRequest = SessionManager.shared.download(request, to: destination)
            .customValidate()
            .response { requestResponse in
    ...
    public func uploadFileRequestTask(patch: URL,
        ...
        let sessionRequest = SessionManager.shared.upload(fromFile, with: request)
            .customValidate()
            .response { requestResponse in
                response(requestResponse.data, requestResponse.response, requestResponse.error)
        }
    ...
    public func uploadFileRequestTask(path: URL,
        ...
        let sessionRequest = SessionManager.shared.upload(fileData, with: request)
            .customValidate()
            .response { requestResponse in
                response(requestResponse.data, requestResponse.response, requestResponse.error)
        }
