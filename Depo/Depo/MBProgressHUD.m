...
- (BOOL) isRemainingLeft {
    return windowProgress < 1.0f;
}

- (float) calculateRemainingTime {
    float counts = (1.0f - windowProgress)/0.025f;
    return 0.05f * counts;
}
...
- (void)show:(BOOL)animated {
	...
	// ... otherwise show the HUD imediately
        ...
        if(iconView) {
            [iconView.layer removeAnimationForKey:@"moveCircle"];
...
- (void)hide:(BOOL)animated {
    ...
    if(windowTimer) {
        [self invalidateWindowTimer];
//        windowProgress = 0.96;
//        [self performSelector:@selector(invalidateWindowTimer) withObject:nil afterDelay:0.1];
    }
