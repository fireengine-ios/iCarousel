//
//  CurrentMusicListModalController.m
//  Depo
//
...
@interface CurrentMusicListModalController ()

@end

@implementation CurrentMusicListModalController
...
//        self.title = NSLocalizedString(@"MusicTitle", @"");
        ...
        musicTable.accessibilityIdentifier = @"musicTableCurrentMusic";
...
- (void) triggerOk {
    [delegate musicModalListReturnedWithSelectedList:self.selectedMusicList];
...
- (void) fileFolderCellDidUnselectFile:(MetaFile *)fileUnselected {
    if([selectedMusicList containsObject:fileUnselected.uuid]) {
        [selectedMusicList removeObject:fileUnselected.uuid];
    }
    if([selectedMusicList count] > 0) {
        self.title = [NSString stringWithFormat:NSLocalizedString(@"FilesSelectedTitle", @""), [selectedMusicList count]];
    } else {
