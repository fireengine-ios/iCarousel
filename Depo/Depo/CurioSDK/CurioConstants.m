//
//  CurioConstants.m
//  CurioSDK
//
//  Created by Can Ciloglu on 30/01/15.
//  Copyright (c) 2015 Turkcell. All rights reserved.
//

#import "CurioConstants.h"



NSString * const CURKeyScreenDataTitle = @"title";
NSString * const CURKeyScreenDataClassName = @"className";
NSString * const CURKeyScreenDataPath = @"path";
NSString * const CURKeySessionCode = @"sessionCode";
NSString * const CURKeyDeviceToken = @"deviceToken";
NSString * const CURKeyLatitude = @"latitude";
NSString * const CURKeyLongitude = @"longitude";
NSString * const CURKeyNotificationId = @"nId";
NSString * const CURKeyLocationId = @"lId";
NSString * const CURKeyActionId = @"aId";
NSString * const CURKeyActionType = @"actionType";
NSString * const CURKeyTimeStamp = @"stamp";
NSString * const CURKeyHitCode = @"hitCode";
NSString * const CURKeyEventKey = @"eventKey";
NSString * const CURKeyEventValue = @"eventValue";
NSString * const CURKeyIsOnline = @"isOnline";
NSString * const CURKeyProperties = @"properties";
NSString * const CURKeyData = @"data";
NSString * const CURKeyPId = @"pId";

NSString * const CURNetworkTypeWifi = @"wifi";
NSString * const CURNetworkTypeMobile = @"mobile";
NSString * const CURNetworkTypeOffline = @"Offline";
NSString * const CURNetworkCarrierUnknown = @"UNKNOWN";

NSString * const CURHttpParamPushToken = @"pushToken";
NSString * const CURHttpParamVisitorCode = @"visitorCode";
NSString * const CURHttpParamTrackingCode = @"trackingCode";
NSString * const CURHttpParamPushId = @"pushId";
NSString * const CURHttpParamCustomId = @"customId";
NSString * const CURHttpParamLatitude = @"lat";
NSString * const CURHttpParamLongitude = @"long";
NSString * const CURHttpParamOffset = @"offset";
NSString * const CURHttpParamLimitSize = @"limitsize";


NSString * const CURDeviceBrandName = @"Apple";

NSString * const CURNotificationTypes = @"Sound,Badge,Alert";
NSString * const CURNotificationTypeSound = @"Sound";
NSString * const CURNotificationTypeBadge = @"Badge";
NSString * const CURNotificationTypeAlert = @"Alert";
