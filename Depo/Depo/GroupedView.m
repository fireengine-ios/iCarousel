    ...
    if(self = [super initWithFrame:frame]) {
        ...
        for(id row in self.group.fileInfo) {
            if([row isKindOfClass:[UploadRef class]]) {
                ...
                SquareImageView *rowImgView = [[SquareImageView alloc] initWithFrame:CGRectMake((counter%imageCountPerRow)*imageWidth, 40 + floorf(counter/imageCountPerRow)*imageWidth, imageWidth, imageWidth) withFile:castedRow withSelectibleStatus:selectFlag shouldCache:NO];
                rowImgView.delegate = self;
                [self addSubview:rowImgView];
                counter++;
            }
...
- (void) loadMoreImages:(NSArray *) moreImages {
    for(id row in moreImages) {
        if([row isKindOfClass:[UploadRef class]]) {
            ...
            MetaFile *castedRow = (MetaFile *) row;
            SquareImageView *rowImgView = [[SquareImageView alloc] initWithFrame:CGRectMake((counter%imageCountPerRowRef)*imageWidthRef, 40 + floorf(counter/imageCountPerRowRef)*imageWidthRef, imageWidthRef, imageWidthRef) withFile:castedRow withSelectibleStatus:isSelectible shouldCache:YES];
            rowImgView.delegate = self;
            [self addSubview:rowImgView];
            counter++;
        }
