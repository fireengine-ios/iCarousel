...
final class InvitationCard: BaseCardView {
  ...
  override func set(object: HomeCardResponse?) {
    ...
    configurateByResponseObject(thumbnailUrl: thumbnailUrl)
  }
    
  private func configurateByResponseObject(thumbnailUrl: String) {
    if let videoUrl = cardObject?.details?["videoUrl"].url {
      playButton.isHidden = false
      self.videoUrl = videoUrl
    }
    if let videoPreviewImageUrl = cardObject?.details?[thumbnailUrl].url {
