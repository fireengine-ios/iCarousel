...
protocol PhotoPrintPhotoSelectionControllerDelegate: AnyObject {
    var selectionState: PhotoPrintPhotoSelectionState { get }
...
enum PhotoPrintPhotoSelectionState {
    case selecting
    case ended
}

final class PhotoPrintPhotoSelectionController: UIViewController, ErrorPresenter {
    
    private weak var delegate: PhotoPrintPhotoSelectionControllerDelegate?
    private let dataSource: PhotoPrintPhotoSelectionDataSourceProtocol
    private var isLoadingMore = false
    private var isLoadingMoreFinished = false
    
    private var selectionState: PhotoPrintPhotoSelectionState {
        return delegate?.selectionState ?? localSelectionState
    }
    
    private var localSelectionState = PhotoPrintPhotoSelectionState.selecting
    private var selectingLimit = 0
    
    private let photosSectionIndex = 0
    private var photos = [SearchItemResponse]()
    private let cellId = String(describing: PhotoPrintPhotoCell.self)
    private let footerId = String(describing: PhotoPrintCollectionSpinnerFooter.self)
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(PhotoPrintPhotoCell.self, forCellWithReuseIdentifier: cellId)
        collectionView.register(PhotoPrintCollectionSpinnerFooter.self,
    ...
    private lazy var loadingMoreFooterView: PhotoPrintCollectionSpinnerFooter? = {
    ...
    required init?(coder aDecoder: NSCoder) {
        assertionFailure()
        self.dataSource = PhotoPrintAllPhotosSelectionDataSource(pageSize: 100)
...
// MARK: - cell updates
extension PhotoPrintPhotoSelectionController {
    ...
    private func updateCellForSelectionState(at indexPath: IndexPath) {
        guard let cell = collectionView.cellForItem(at: indexPath) as? PhotoPrintPhotoCell else {
            return
        }
    ...
    private func updateVisibleCellsForSelectionState() {
        let cells = collectionView.indexPathsForVisibleItems.compactMap({ collectionView.cellForItem(at: $0) as? PhotoPrintPhotoCell })
        cells.forEach({ $0.update(for: selectionState) })
    }
}

// MARK: - UICollectionViewDataSource
extension PhotoPrintPhotoSelectionController: UICollectionViewDataSource {
...
extension PhotoPrintPhotoSelectionController: UICollectionViewDelegate {
    
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let cell = cell as? PhotoPrintPhotoCell else {
            assertionFailure()
            return
        }
...
extension PhotoPrintPhotoSelectionController: UICollectionViewDelegateFlowLayout {
...
extension PhotoPrintPhotoSelectionController: PhotoPrintSelectionSegmentedControllerDelegate {
...
extension PhotoPrintPhotoSelectionController: ReachabilityServiceDelegate {
    func reachabilityDidChanged(_ service: ReachabilityService) {
        ...
        for indexPath in collectionView.indexPathsForVisibleItems {
            guard
                let cell = collectionView.cellForItem(at: indexPath) as? PhotoPrintPhotoCell,
