    ...
    @objc private func didLongTapChange(_ gestureRecognizer: UILongPressGestureRecognizer) {
        ...
        case .began:
            if let indexPath = pointedIndexPath, let cellIsSelected = cellForRow(at: indexPath)?.isSelected {
                ...
                selectedOriginallyIndexPaths = indexPathsForSelectedRows ?? []
...
//        print("negative range: \(negativeIndexPaths)")
    ...
    private func indexPaths(in range: ClosedRange<IndexPath>) -> [IndexPath] {
        ...
        let numberOfItemsInSection = dataSource.tableView(self, numberOfRowsInSection: beginSection)
