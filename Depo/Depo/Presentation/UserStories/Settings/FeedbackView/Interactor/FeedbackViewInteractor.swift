...
class FeedbackViewInteractor: FeedbackViewInteractorInput {
    ...
    func onSend(selectedLanguage: LanguageModel){
        ...
        FeedbackService().sendSelectedLanguage(selectedLanguageParameter: parameter, succes: {[weak self] (success) in
            DispatchQueue.main.async {
                self?.getUserInfoString(with: selectedLanguage.languageCode!)
            }
    ...
    func getUserInfoString(with languageCode: String){
        ...
        group.notify(queue: queue) {[weak self] in
            DispatchQueue.main.async {
                ...
                let packages = subscriptions
                    ...
                    .joined(separator: ", ")
                let userInfoString = String(format: "Application Version: %@\nMsisdn: %@\nCarrier: %@\nDevice:%@\nDevice OS: %@\nLanguage: %@\nNetwork Status: %@\nTotal Storage: %lld\nUsed Storage: %lld\nPackages: %@\n", versionString, phoneString, CoreTelephonyService().operatorName() ?? "" , UIDevice.current.model, UIDevice.current.systemVersion, languageCode, ReachabilityService().isReachableViaWiFi ? "WWAN" : "WIFI", quota, quotaUsed, packages)
