...
//
//  Source: https://github.com/stleamist/MKZoomLevel
...
public extension MKMapView {
    ...
    func setZoomLevel(_ zoomLevel: CGFloat, animated: Bool) {
        let newCenterCoordinateDistance = centerCoordinateDistance(of: zoomLevel)
    ...
    @available(iOS 13.0, *)
    func setCameraZoomRange(minimumZoomLevel: CGFloat, maximumZoomLevel: CGFloat, animated: Bool) {
        let range = MKMapView.CameraZoomRange(
            minCenterCoordinateDistance: centerCoordinateDistance(of: maximumZoomLevel),
            maxCenterCoordinateDistance: centerCoordinateDistance(of: minimumZoomLevel)
        )
        setCameraZoomRange(range, animated: animated)
    }
    ...
    private func centerCoordinateDistance(of zoomLevel: CGFloat) -> CLLocationDistance {
        ...
        return oldCenterCoordinateDistance * (newBottomLongitudeDelta / oldBottomLongitudeDelta)
    }
...
extension MKMapView {
    ...
    var southEast: CLLocationCoordinate2D {
        return convert(CGPoint(x: bounds.width, y: bounds.height), toCoordinateFrom: self)
    }
}
