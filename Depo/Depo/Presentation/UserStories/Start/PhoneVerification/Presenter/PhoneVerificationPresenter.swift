...
class PhoneVerificationPresenter: BasePresenter, PhoneVerificationModuleInput, PhoneVerificationViewOutput, PhoneVerificationInteractorOutput {
    ...
    func resendCodeRequestFailed(with error: ErrorResponse) {
        ...
        if case ErrorResponse.error(let containedError) = error {
            if containedError.description == localized(.tooManyRequests) {
                let popUp = PopUpController.withDark(title: TextConstants.errorAlert,
                                                 message: localized(.tooManyRequests),
                                                 image: .custom(Image.forgetPassPopupLock.image),
                                                 buttonTitle: TextConstants.ok)
                popUp.open()
            } else if let serverError = containedError as? ServerError, serverError.code == 401 {
